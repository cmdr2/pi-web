...
- **Session workspace**: browse, resume, rename, export, and delete conversations grouped by project, with running state, context usage, cost, and compaction details. The top bar shows the current session's cost followed by the total cost of every session in that project folder in brackets, e.g. `$1.23 ($4.56)`.
