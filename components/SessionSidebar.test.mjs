...
test("exposes the unread-session markers to the shell", () => {
  assert.match(source, /onUnreadSessionIdsChange\?: \(ids: Set<string>\) => void/);
  assert.match(source, /onUnreadSessionIdsChange\?\.\(unreadSessionIds\)/);
});
...
test("lifecycle refreshes bypass the cache while cross-window polling reuses it", () => {
  ...
  assert.match(source, /loadSessions\(!isFirst\)/);
  ...
  assert.match(source, /loadSessions\(true\);[\s\S]*?onBackgroundTaskDone/);
});

test("drops the New button, session search and session list from the sidebar", () => {
  assert.doesNotMatch(source, /onNewSession/);
  assert.doesNotMatch(source, /handleNewSession/);
  assert.doesNotMatch(source, /sessionSearchOpen|sessionSearchQuery|SessionSearch/);
  assert.doesNotMatch(source, /sidebar\.toggleSessionSearch/);
  assert.doesNotMatch(source, /function SessionItem\(|listSessionFamilies|getSessionListIndices/);
  assert.doesNotMatch(source, /SESSION_LIST_ITEM_HEIGHT/);
  assert.doesNotMatch(source, /data-\w*=["']?session-search-input/);
});