...
test("runtime session info carries the summed usage cost of its entries", (t) => {
  const previousRegistry = globalThis.__piSessions;
  const timestamp = "2026-08-12T01:02:03.000Z";
  const usage = (total) => ({ usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total } } });
  const runtime = makeRuntimeSession({
    id: "costed-runtime",
    filePath: join(tmpdir(), "pi-web-missing-costed-session.jsonl"),
    running: true,
    entries: [
      { type: "message", id: "u1", parentId: null, timestamp, message: { role: "user", content: "hello" } },
      { type: "message", id: "a1", parentId: "u1", timestamp, message: { role: "assistant", content: [{ type: "text", text: "hi" }], ...usage(0.25) } },
      { type: "message", id: "t1", parentId: "a1", timestamp, message: { role: "toolResult", toolCallId: "c1", content: [{ type: "text", text: "ok" }], ...usage(0.125) } },
      { type: "compaction", id: "c1", parentId: "t1", summary: "s", firstKeptEntryId: "u1", tokensBefore: 5, timestamp, ...usage(0.0625) },
    ],
  });
  globalThis.__piSessions = new Map([["costed-runtime", runtime]]);
  t.after(() => {
    globalThis.__piSessions = previousRegistry;
  });
  ...
  assert.equal(infos.length, 1);
  assert.equal(infos[0].cost, 0.4375);
});
