...
// `cost` is a pi-web extension beyond the SDK metadata; strip it where the
// tests assert exact equality with the SDK catalogue.
const withoutCost = (sessions) => sessions.map((session) => {
  const rest = { ...session };
  delete rest.cost;
  return rest;
});
const usage = (total) => ({ usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, cost: { input: total / 2, output: total / 2, cacheRead: 0, cacheWrite: 0, total } } });
const assistant = (id, cost, time = timestamp) => ({
  type: "message", id, parentId: null, timestamp: time,
  message: { role: "assistant", content: [{ type: "text", text: "answer" }], ...(cost === undefined ? {} : usage(cost)) },
});
...
test("matches SDK metadata and tie ordering across cache hits, mutations, and restarts", async (t) => {
  ...
  async function check(label, expectedReads) {
    ...
    opened.length = 0;
    assert.deepEqual(withoutCost(await listSessionsIncremental()), expected, label);
...
test("retains SDK handling of partial lines, activity timestamps, and empty sessions", async (t) => {
  ...
  assert.deepEqual(withoutCost(await listSessionsIncremental()), await sdkMetadata());
});
...
test("sums usage cost across all entry kinds and caches it across restarts", async (t) => {
  const { write } = fixture(t);
  const a = write("a", [
    message("a1", "user", "request"),
    assistant("a2", 0.25),
    { ...assistant("a3", 0.5), message: { ...assistant("a3").message, role: "toolResult", toolCallId: "t1", content: [{ type: "text", text: "result" }], ...usage(0.5) } },
    { type: "compaction", id: "a4", parentId: "a3", summary: "summary", firstKeptEntryId: "a1", tokensBefore: 10, timestamp, ...usage(0.125) },
  ]);
  write("b", [message("b1", "user", "other project session"), assistant("b2", 1.5)]);

  const sessions = await listAllSessions({ force: true });
  const byId = new Map(sessions.map((s) => [s.id, s]));
  assert.equal(byId.get("a").cost, 0.875);
  assert.equal(byId.get("b").cost, 1.5);

  // Persisted index keeps serving the same costs without rescanning.
  resetSessionScanIndexForTests();
  const cached = await listAllSessions();
  const cachedById = new Map(cached.map((s) => [s.id, s]));
  assert.equal(cachedById.get("a").cost, 0.875);
  assert.equal(cachedById.get("b").cost, 1.5);

  // Appending usage updates the cached cost after rescan.
  fs.appendFileSync(a, line(assistant("a5", 0.05, "2026-01-02T00:00:00.000Z")));
  const refreshed = await listAllSessions({ force: true });
  const refreshedById = new Map(refreshed.map((s) => [s.id, s]));
  assert.equal(refreshedById.get("a").cost, 0.925);
});

test("v1 index entries without cost are discarded and rebuilt", async (t) => {
  const { write, indexPath } = fixture(t);
  write("a", [message("a1", "user", "request"), assistant("a2", 0.4)]);
  await listSessionsIncremental();
  const index = JSON.parse(fs.readFileSync(indexPath, "utf8"));
  assert.equal(index.version, 2);
  delete index.entries[Object.keys(index.entries)[0]].info.cost;
  fs.writeFileSync(indexPath, JSON.stringify(index));
  resetSessionScanIndexForTests();

  const sessions = await listSessionsIncremental();
  assert.equal(sessions.length, 1);
  assert.equal(sessions[0].cost, 0.4);
});
